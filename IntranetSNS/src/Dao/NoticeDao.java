...
해당 클래스 주요 기능
- 공지사항 테이블에서 등록된 공지사항을 가져오는 메서드를 포함한 클래스
...
public class NoticeDao {
	...
	public void getAllNotice(ObservableList<NoticeTableView> list) {
		...
		try {
			...
			while (rs.next()) {
				list.add(new NoticeTableView(rs.getInt("noticeno"), rs.getString("noticeclass"),
	...
	public Notice getSelectedNotice(String noticeNo) {
		...
		try {
			...
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				notice = new Notice(rs.getString("noticeclass"), rs.getString("noticetitle"),
