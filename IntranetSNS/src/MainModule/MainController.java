...
public class MainController implements Initializable {
	// 왼쪽 영역
	@FXML
	private ToggleButton toggleBtnNotice, toggleBtnUser, toggleBtnChat, toggleBtnBoard; // 왼쪽 영역 버튼
	@FXML
	private ToggleGroup groupCategory; // 왼쪽 영역 버튼 토글 그룹
	@FXML
	private Button btnSchedule; // 왼쪽 영역 일정 버튼

	// 오른쪽 영역
	@FXML
	private ComboBox<String> comboBoxSideFilter; // 오른쪽 영역 부서선택 콤보박스
	@FXML
	private TableView<User> tblViewSideUserList; // 오른쪽 영역 사용자 목록 테이블뷰
	@FXML
	private TextField txtFieldSideFilter; // 오른쪽 영역 이름 검색 필드

	// 가운데 영역
	@FXML
	private AnchorPane anchorPaneNotice, anchorPaneUser, anchorPaneChat, anchorPaneBoard;
	// 가운데의 각 영역을 표시하는 팬

	// 알림
	@FXML
	private Label lblMainNoticeTitle, lblMainNoticeContent; // 최상단의 공지사항 부분
	@FXML
	private Button btnNoticeRefresh; // 새로고침 버튼
	@FXML
	private TableView<NoticeTableView> tblViewNotice;
	@FXML
	private Button btnFold; // 공지사항을 접는 버튼
	@FXML
	private HBox boxMainNotice;

	// 사용자정보
	@FXML
	private AnchorPane anchorPaneStackedPane; // 내 정보 카드 더블 클릭 때 사용
	@FXML
	private Button btnUserRefresh; // 사용자 상세 목록 새로고침 버튼
	@FXML
	private ImageView imgViewUserImg;
	@FXML
	private TabPane tabPaneUser; // 사용자 탭 목록 영역
	@FXML
	private TextField txtFieldUserFilter; // 사용자 탭 이름 검색 필드
	@FXML
	private Label lblMyDept, lblMyName, lblMyPosition, lblMyStatusMsg;

	// 채팅
	@FXML
	private Button btnOpenChat, btnDeptChat;

	// 게시판
	@FXML
	private Button btnWrite, btnBoardRefresh; // 게시물 탭 글쓰기, 새로고침 버튼
	@FXML
	private TableView<BoardTableView> tblViewBoardList; // 게시판 탭 테이블뷰
	@FXML
	private ComboBox<String> comboBoxBoardFilter;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		comboBoxSideFilter.getSelectionModel().selectedItemProperty()
				.addListener((obs, oldValue, newValue) -> sideFilterAction());
		...
		// 탭 선택에 따른 동작
		tabPaneUser.getSelectionModel().selectedItemProperty()
				.addListener((obs, oldValue, newValue) -> centerFilterAction());
			...
			public TableCell<User, User> call(TableColumn<User, User> param) {
				return new TableCell<User, User>() {
					...
					private Label dept, name, position;
					{
						...
						name.setStyle(
								"-fx-pref-width:60; -fx-font-size:14; -fx-font-weight:bold; -fx-alignment:center_left;");
	...
	public void sideFilterAction() {
		if (txtFieldSideFilter.getText().isEmpty() || txtFieldSideFilter.getText().equals("")) { // 검색필드가 비어있다면
			if (comboBoxSideFilter.getSelectionModel().getSelectedItem().equals("전체")) { // 콤보박스를 전체로 선택하면 모든 정보 불러옴
				...
				userInfoDao.loadFilteredAllUser("right", sideTblViewUserList, USER_NO,
						comboBoxSideFilter.getSelectionModel().getSelectedItem());
			}
		} else { // 필드에 무언가가 적혀있다면
			if (comboBoxSideFilter.getSelectionModel().getSelectedItem().equals("전체")) { // 필드에 적혀있고 전체를 선택하면 필드를 기준으로
																							// 결과 불러옴
				...
				userInfoDao.loadFilteredAllUser("right", sideTblViewUserList, USER_NO,
						comboBoxSideFilter.getSelectionModel().getSelectedItem(), txtFieldSideFilter.getText());
			}
		}
	}
	...
	public TableView<User> createUserInfoTable(String tabName, String filterTxt) {
		...
		userInfoCol
				.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<User, User>, ObservableValue<User>>() {
					@Override
					public ObservableValue<User> call(CellDataFeatures<User, User> param) {
						return new ReadOnlyObjectWrapper<User>(param.getValue());
					}
			...
			public TableCell<User, User> call(TableColumn<User, User> param) {
				return new TableCell<User, User>() {
					...
					private Label dept, name, position, msg;
					{
						...
						dept.setStyle(
								"-fx-pref-width:70; -fx-pref-height:30; -fx-font-size:15px; -fx-alignment:center_left;");
						name.setStyle(
								"-fx-pref-width:80; -fx-pref-height:30; -fx-font-size:20px; -fx-alignment:center_left; -fx-font-weight:bold;");
						position.setStyle(
								"-fx-pref-width:50; -fx-pref-height:30; -fx-font-size:14px; -fx-alignment:center_left;");
						msg.setStyle(
								"-fx-pref-width:200; -fx-pref-height:50; -fx-font-size:12px; -fx-alignment:center_left; -fx-padding:5");
					}
	...
	public void createBoardTable(TableView<BoardTableView> boardTable) {
		...
		writerDateCol.setCellValueFactory(
...
//		User user = userInfoDao.selectMyInfo(USER_NO);
	...
	public void createNoticeTable(TableView<NoticeTableView> noticeTable) {
		...
		classCol.setStyle(
				"-fx-pref-width:80; -fx-border-width:1; -fx-pref-height:40; -fx-font-size:10px; -fx-alignment:center");
		...
		TableColumn<NoticeTableView, NoticeTableView> noticeContentCol = new TableColumn<NoticeTableView, NoticeTableView>(
				"알림 내용");
		noticeContentCol
				.setStyle("-fx-pref-width:220; -fx-border-width:1; -fx-pref-height:40; -fx-alignment:center-left");
		// 사용자 지정형태
		noticeContentCol.setCellValueFactory(
				new Callback<CellDataFeatures<NoticeTableView, NoticeTableView>, ObservableValue<NoticeTableView>>() {
					@Override
		...
		noticeContentCol.setCellFactory(
				new Callback<TableColumn<NoticeTableView, NoticeTableView>, TableCell<NoticeTableView, NoticeTableView>>() {
					@Override
