...
public class CreateScheduleController implements Initializable {
	...
	@FXML
	Button btnReg, btnCancel, btnDel;
	@FXML
	Label lblDate;
	@FXML
	TableView<Reg> tblViewPrivateSch, tblViewGroupSch;
	...
	public void handleBtnRegAction() {
		if (tblViewPrivateSch.getSelectionModel().isEmpty()) {
			sd.entrySchedule(
					new Reg(MainController.USER_NO, txtFieldTitle.getText(), txtAreaContent.getText(), today, null));
		} else {
			sd.updateSchedule(
					new Reg(MainController.USER_NO, txtFieldTitle.getText(), txtAreaContent.getText(), today, null));
		}
	...
	public void handleBtnDelAction() {
		sd.deleteSchedule(
				new Reg(MainController.USER_NO, txtFieldTitle.getText(), txtAreaContent.getText(), today, "1"));
		privateList.clear();
		sd.loadPrivateSchedule(privateList, MainController.USER_NO, today);
		tblViewPrivateSch.setItems(privateList);
		txtFieldTitle.clear();
