...
public class DayOffApplyController implements Initializable {
	@FXML
	private DatePicker pickerDayOffStart, pickerDayOffFinish;
	@FXML
	...
	public void handleBtnApplyAction() {
		...
		if (dateStart == null || dateFinish == null) { // 날짜 선택을 하지 않았을 때
		...
		} else if (dateStart.until(dateFinish).isNegative() || dateStart.until(dateFinish).isZero()) { // 끝나는 날짜가 시작
																										// 날짜보다 앞 일 경우
			...
			scheduleDao.entryDayOff(new DayOff(userid, pickerDayOffStart.getPromptText(),
					pickerDayOffFinish.getPromptText(), txtAreaReason.getText()));
