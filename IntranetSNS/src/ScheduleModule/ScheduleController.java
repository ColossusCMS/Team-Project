...
public class ScheduleController implements Initializable {
	@FXML
	RadioButton radioBtnAll, radioBtnPrivate, radioBtnGroup;
	@FXML
	ToggleGroup radioGroupSchedule;
	@FXML
	Button btnPrevMonth, btnNextMonth, btnDayOffApply;
	@FXML
	GridPane gridPaneCalendar;
	@FXML
	Label lblYear, lblMonth, lblMonthTxt;
	...
	/*
	 * 날짜를 사용하는데 Calendar와 Date를 사용하지 않은 이유 1. 불변 객체가 아니기 때문에 set으로 변경을 했을 때 올바른 값이
	 * 출력되지 않을 수 있다는 문제점이 있다. 2. 상수 필드 남용 3. 월표기가 0부터 시작하는 것 4. 요일 상수에 일관성이 없다. 어디서는
	 * 일요일이 0, 어디서는 일요일이 1 5. Date와 Calendar객체의 역할 분담.
	 */
			...
			for (int j = 0; j < 7; j++) { // 열 수
				...
				if (radioBtnAll.isSelected()) { // 전체 일정으로 선택했을 때
					...
					if (privateSchList.contains(daysList.get(index) + "")
							&& groupSchList.contains(daysList.get(index) + "")) {
